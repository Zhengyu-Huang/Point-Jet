...
def heat_solve(tau, omega_jet, dt = 1.0, Nt = 1000, save_every = 1, L = 4*np.pi):
    nu = 0.1
    # set top and bottom Dirichlet condition
...
for tau in [10.0, 20.0]:
    for Gamma in [1.0, 2.0]:
        for jet_type in ["Point", "Gaussian"]:


            test_name = "Jet type = %s Gamma=%1.1f tau=%1.1f" %(jet_type, Gamma, tau)
            ...
            np.save(test_name+"/"+"omega.npy", omega_data)
            np.save(test_name+"/"+"M1.npy", M1)
            np.save(test_name+"/"+"M2.npy", M2)

            plt.savefig(test_name + "/" + test_name + ".png")
            # animate(yy, t_data, omega_data)